...
class PollFlashListener extends Listener
{
	function eventOccured(Event $event)
	{
		...
		LEFT JOIN flashmail AS f ON (f.to_user_id=o.user_id AND f.message = CONCAT(".$message.",p.question)) AND f.from_user_id=1
		LEFT JOIN poll_votes AS v ON v.user=o.user_id AND v.poll=p.id 
		WHERE f.message IS NULL AND p.id=(SELECT MAX(id) FROM polls) AND v.id IS NULL ;");
	}
}
