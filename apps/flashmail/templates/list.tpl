	...
	{foreach item=flashmail from=$flashmails}
		...
		<li class="read">
		{/if}
			<span class="answerlink"><a href="{kurl app="flashmail" page="write" flashmailid=$flashmail->getId()}" onclick="new Ajax.Request('{kurl app="flashmail" page="setasread" flashmailid=$flashmail->getId()}', {literal}{asynchronous:true, evalScripts:true}{/literal}); flashmail_headerbox_update(); new Ajax.Updater('flashmail_headerbox_answer', '{kurl app="flashmail" page="headerbox_answer" flashmailid=$flashmail->getId()}',{literal} {asynchronous:true, evalScripts:true}{/literal}); Effect.Appear('flashmail_headerbox_answer'); return false;">##REPLY##</a></span>
			<span id="account_flashmail_read_link" class="readlink"><a href="{kurl app="flashmail" page="setasread" flashmailid=$flashmail->getId()}" onclick="new Ajax.Request('{kurl app="flashmail" page="setasread" flashmailid=$flashmail->getId()}', {literal}{asynchronous:true, evalScripts:true}{/literal}); flashmail_headerbox_update(); return false;">##SETASREAD##</a></span>
