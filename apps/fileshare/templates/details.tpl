...
{if $myElement == NULL}
<div class="hint hint_error">##FS_FILEDOESNOTEXIST##</div>
{else}
	<h3>{if $myElement->isFile()}##FILEDETAILS##{else if ($myElement->isDirectory())}##DIRECTORYDETAILS##{/if} : {$myElement->getName()}</h3>
	...
	<div class="fileshare">
		...
		<div class="details{if ($myElement->isFile() && ($myElement->getExtension() != ""))} {$myElement->getExtension()|lower}{elseif $myElement->isDirectory()} directory{/if}">
			...
			<div class="detail groupowner">
				...
				<span id="groupowner">
					<a href="{kurl app="annuaire" page="groupid" id=$myElement->getSysInfos("groupowner")}">{$myElement->getSysInfos("groupowner_name")}</a>&nbsp;
				</span>
			</div>
				{/if}
			{/if}
