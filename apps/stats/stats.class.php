...
class Stats extends Model
{
	...
	public function build()
	{
		...
		try {
			$this->db->exec("CREATE VIEW minichat_stats_temp AS (SELECT id_auteur AS auteur, post, DATE(`time`) AS msgDate, `time` AS msgTime FROM minichat WHERE post LIKE 'preums' OR post LIKE 'deuz' OR post LIKE 'troiz' OR post LIKE 'dernz' ORDER BY `time`)");
			$this->db->exec("CREATE VIEW minichat_preums AS (SELECT auteur, msgDate FROM minichat_stats_temp WHERE post LIKE 'preums' GROUP BY msgDate ORDER BY msgTime)");
			$this->db->exec("CREATE VIEW minichat_deuz AS (SELECT mt.auteur, mt.msgDate FROM minichat_stats_temp mt WHERE mt.post LIKE 'deuz' AND NOT(mt.auteur IN (SELECT mp.auteur FROM minichat_preums mp WHERE mp.msgDate = mt.msgDate)) GROUP BY mt.msgDate ORDER BY msgTime)");
			$this->db->exec("CREATE VIEW minichat_troiz AS (SELECT mt.auteur, mt.msgDate FROM minichat_stats_temp mt WHERE mt.post LIKE 'troiz' AND NOT(mt.auteur IN (SELECT mp.auteur FROM minichat_preums mp WHERE mp.msgDate = mt.msgDate)) AND NOT(mt.auteur IN (SELECT md.auteur FROM minichat_deuz md WHERE md.msgDate=mt.msgDate)) GROUP BY mt.msgDate ORDER BY msgTime);");
