...
class OnlineUsersList extends Model
{
	public function build()
	{
		...
		if ($this->currentUser->isLogged()) {	
			$sql = "SELECT message, mood FROM usermood WHERE user_id=:user";
			try {
				$stmt = $this->db->prepare($sql);
			...
			$sql = "SELECT p.gender FROM " . $GLOBALS['config']['bdd']["frameworkdb"] . ".users u LEFT JOIN " . $GLOBALS['config']['bdd']["frameworkdb"] . ".profile p ON p.id=u.profile_id WHERE u.id=:user";
			$isGirl = false;
			try {
